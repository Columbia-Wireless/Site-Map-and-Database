...
**Field definition still open, asked of Onno 2026-08-20.** What we actually want extracted isn't fully captured yet: management commission (percentage or flat fee Columbia Wireless takes), billing practices (who invoices/collects/remits rent and how), agreement start date, agreement end date/term length, and whether one agreement covers a single site or a defined multi-site portfolio (and if so, which sites — SAM 2.0's payload is one-document-one-site today, so multi-site coverage needs a decision on whether SAM 2.0 files the same document once per covered site or something else). `TermsReviewModal`'s `TERM_GROUPS` already has placeholder slots for `management_commission` and `billing_practices`, unpopulated until the payload supports them.
