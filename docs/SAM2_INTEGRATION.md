...
Sam2SyncPayload {
  ...
  extractedData: {
    ...
    legalTerms?: { premisesDescription?, governingLaw?, permittedUse?,
                   assignmentAllowed?: 'yes'|'no'|'conditional',
                   terminationNoticeDays?: number,  // days, NOT months — distinct from renewalOptions.noticePeriodMonths
                   relocationProvisions?, equipmentDescription?, notes? }
  }
...
- **2026-08-20 field parity update.** We compared the legacy in-house extractor's field list against this payload and sent Onno a gap list (`utilities`/`holdover`/`insuranceRequirements` turned out to already exist, just undocumented here — now added above and wired into `extracted_terms` as `utilities`/`holdover_provisions`/`insurance_per_occurrence`/`insurance_aggregate`/`insurance_liability`). Onno also shipped a brand-new `legalTerms` block the same day (premises description, governing law, permitted use, assignment allowed, termination notice days, relocation provisions, equipment description, catch-all notes) — confirmed shape received and wired in same day (`premises_description`/`governing_law`/`permitted_use`/`assignment_allowed`/`termination_notice_days`/`relocation_provisions`/`equipment_description`/`notes` in `extracted_terms`). `terminationNoticeDays` is in **days**, unlike `leaseTerms.renewalOptions.noticePeriodMonths` which is in months — deliberately not the same unit, don't conflate them. `assignmentAllowed` is a three-state enum (`'yes'|'no'|'conditional'`), not a boolean. Forward-only: documents already filed before this change keep their old extraction, no bulk reprocessing tool exists yet.
...
Between a site owner and Columbia Wireless itself, most often per-site but sometimes covering multiple sites under one agreement (client-confirmed 2026-08-20) — this engages Columbia Wireless to market and manage the property's telecom sites and negotiate carrier leases on the owner's behalf. No carrier, no rent terms of the lease kind. SAM 2.0 flags these via `extractedData.classification.role === 'management_agreement'`.

**Bug fixed 2026-08-20.** Before this, these fell through the normal lease path: `siteIdentity.lesseeName` (Columbia Wireless, the manager) was read as if it were a carrier, creating a phantom `licensees` record and a `$0` `site_licenses` row for every management agreement synced. Now handled like non-instrument documents for site/licensee/license purposes — stored and attached to the site, no licensee or license record created, no owner-match attempted (a management agreement's stated owner is never trusted for site ownership resolution, same client-confirmed rule as any other non-instrument document, 2026-08-04).

Unlike non-instrument documents, a management agreement is a real contract with real terms worth capturing, so it still goes through the normal `review_required` gate and `TermsReviewModal`, just with its own minimal field set for now: `document_category`, `site_owner` (from `siteIdentity.lessorName`), `manager_entity` (from `siteIdentity.lesseeName`), `signature_date`, `commencement_date`, plus `governing_law`/`termination_notice_days`/`notes` when `legalTerms` is populated for this doc type.

**Field definition confirmed and wired 2026-08-20.** Onno shipped a `managementTerms` block (only present on `management_agreement` documents): `commissionStructure` ('percentage'|'flat_fee'|'hybrid'|'other'), `commissionPercentage` (fraction, display/cross-check only, see below), `commissionFlatFeeAmount`, `commissionDescription`, `billingPractices`, `termEndDate` (only if explicit, never computed from term length), `initialTermDescription` (fallback prose when there's no explicit end date), `renewalTerms`, `terminationNoticeDays`, `exclusivity` ('exclusive'|'non_exclusive'), and `coversMultipleSites` (boolean, never null, defaults true when genuinely unclear). Start date needed no new field — `documentMetadata.effectiveDate` already covers it.

**Important:** `managementTerms.commissionPercentage` is NOT the operational commission rate the rent engine uses. It's what the document states, stored on `management_agreements.commission_rate` for display/cross-check only — nothing writes it into any calculation input automatically. That's a separate, manually-confirmed number (Onno's explicit warning).

**Multi-site answer:** SAM 2.0 never files a management agreement against more than one site today. A document naming multiple properties stays in Onno's review inbox for a person to sort out manually (same behavior a portfolio-wide lease already gets) — we will only ever receive single-site payloads here, despite `management_agreement_sites` being a join table. `coversMultipleSites` is stored but should never actually be `true` in a payload we receive.

Now written into `management_agreements`/`management_agreement_sites` (`supabase/management_agreements.sql`) by `/api/sam2/sync`, non-fatally — if that migration hasn't been run against the live database yet, the write is caught and warned on, not allowed to break the underlying document sync. `TermsReviewModal`'s "Management Agreement" `TERM_GROUPS` entry now includes `exclusivity`, `term_end_date`, and `renewal_terms` alongside the earlier fields.
