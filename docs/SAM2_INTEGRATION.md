...
2. **Correction write-back (us to SAM 2.0) — also doesn't exist yet.** An intake endpoint for corrections, auth/API key for our outbound calls, the expected payload shape (field path, old value, new value, who, when), and what SAM 2.0 does when a correction arrives (stored as override, triggers re-validation, needs human review).
